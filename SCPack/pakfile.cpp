...
namespace scpak
{
    ...
    void PakFile::save(std::ostream &stream)
    {
        ...
        PakHeader header;
        header.contentCount = (int)m_contents.size();
        ...
        header.contentOffset = (int)stream.tellp();
        // write content items
        for (PakItem &item : m_contents)
        {
            // there is a magic number before every content data in origin Content.pak
            // it's DEADBEEF
            stream.put((char)0xDE);
            stream.put((char)0xAD);
            stream.put((char)0xBE);
            stream.put((char)0xEF);
        ...
        header.fileLength = (int32_t)stream.tellp();
